			...
			if ((($regs[1] == $parse_month) && ($printview == 'month')) || (($key == $getdate) && ($printview == 'day')) || ((($key >= $week_start) && ($key <= $week_end)) && ($printview == 'week')) || ((substr($regs[1],0,4) == $parse_year) && ($printview == 'year'))) {
				...
				foreach ($val as $new_val) {
					...
					if ($new_val2['event_text']) {	
						...
						// Check it should be taken in the filter
						foreach( $filters as $filter ){
							$filter_without_semicolons = str_replace([' :',':'], '', $filter );

							if(stristr($event_text,$filter) || stristr(str_replace(' :',':',$event_text), $filter) || strtolower(substr($event_text, 0, strlen($filter_without_semicolons))) === $filter_without_semicolons ){
						...
						foreach ($event_times as $uid => $val) {
							...
							if (!isset($val['event_start'])) {
								...
								if ($type == 'large') {
									...
									$switch['EVENT'] = '<a href="day.php?cal=' . $cal . '&getdate=' . $daylink . '" class="tooltip day-summary" data-tooltip-content="#tooltip_' . $daylink . '">' . $nb_events . '</a>';
